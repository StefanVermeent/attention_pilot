...
study2_prim_aim1_effect_plot <- unique(study2_prim_aim1_effects_sum$dv_iv) |> 
  map(function(x) {
    ...
    iv <- str_remove(x, "^.*\\.")
    estimate_var <- paste0('estimate_', iv) |> sym()
    med_effect_var <- paste0('med_effect_', iv) |> sym()
    pvalue_var_chr <- paste0('p.value_', iv, "_chr") |> sym()
    
    effect_data <- study2_prim_aim1_effects_sum |> 
      filter(dv_iv == x) |> 
      select(decision, dv_iv, term, estimate, p.value) |> 
      mutate(term = ifelse(term == "(Intercept)", "intercept", term)) |> 
      pivot_wider(names_from = term, values_from = c(estimate, p.value)) |> 
      mutate(
        across(
          starts_with('p.value'), ~ifelse(. < .05, "sig", "non-sig"), .names = "{.col}_chr"
        )
      )
    
    iv_min <- cleaned_data |> pull(iv) |> scale() |>  min()
    iv_max <- cleaned_data |> pull(iv) |> scale() |>  max()
    y_min <- min(effect_data$estimate_intercept) + iv_max*min(effect_data[paste0("estimate_", iv)]) -0.05
    y_max <- max(effect_data$estimate_intercept) + iv_max*max(effect_data[paste0("estimate_", iv)]) +0.05
    int_med <- median(effect_data$estimate_intercept)
    
    ggplot() +
      geom_segment(data = effect_data, aes(x = iv_min, xend = iv_max, y = int_med + !!estimate_var*iv_min, yend = int_med + !!estimate_var*iv_max, color = !!pvalue_var_chr)) +
...
study2_prim_aim1_eff_curve_plot <- unique(study2_prim_aim1_effects_sum$dv_iv) |> 
  map(function(x) {
    ...
    study2_prim_aim1_effects_sum |> 
      ...
      geom_text(
        aes(x = midpoint, y = 0.28, label = paste0(round(sum(p.value < .05)/n*100,1), " % of p-values < .05")),
        size = 3,
        show.legend = F,
        inherit.aes = F
      ) +
...
study2_prim_aim1_pvalues_plot <- unique(study2_prim_aim1_effects_sum$dv_iv) |>
  map(function(x) {
    ...
    pvalues |> 
      ...
      geom_text(
        data = pvalues %>% summarize(p = paste0(as.character(round(sum(p.value < .05)/n()*100)), " % of ps < .05"), x = (min(p.value)+max(p.value))/2, y = 15),
        aes(x = x, label = p, y = 25),
        size = 5,
        hjust = 0,
        vjust = 1,
        show.legend = F,
      ...
      theme(
        ...
        axis.title.y    = element_text(size = 16, angle = 0, vjust = .5,margin = margin(0,0.25,0,0, "lines")),
...
study2_prim_aim2_eff_curve_plot <- unique(study2_prim_aim2_effects_sum$dv_iv) |> 
  map(function(x) {
    ...
    study2_prim_aim2_effects_sum |> 
      ...
      geom_text(
        aes(x = midpoint, y = 0.18, label = paste0(round(sum(p.value < .05)/n*100,1), " % of p-values < .05")),
        size = 3,
        show.legend = F,
        inherit.aes = F
      ) +
...
study2_aim1_plot <- 
  (study2_prim_aim1_effect_plot$p_flanker.vio_comp + ylim(0.4,0.6) + ggtitle("Perceptual input") + theme(plot.title = element_text(hjust = 0.5, size = rel(0.9), face = "bold")) +
     study2_prim_aim1_effect_plot$p_flanker.unp_comp  + ylim(0.4,0.6) + ggtitle("Perceptual input") + theme(plot.title = element_text(hjust = 0.5, size = rel(0.9), face = "bold")) +
     study2_prim_aim1_effect_plot$interference_flanker.vio_comp  + ylim(80,130) + ggtitle("Interference") + theme(plot.title = element_text(hjust = 0.5, size = rel(0.9), face = "bold")) +
     study2_prim_aim1_effect_plot$interference_flanker.unp_comp  + ylim(80,130) + ggtitle("Interference") + theme(plot.title = element_text(hjust = 0.5, size = rel(0.9), face = "bold")) + plot_layout(ncol = 4)) /
  # Row 2: Effect curves
  ((study2_prim_aim1_eff_curve_plot$p_flanker.vio_comp + ggtitle("Effect curves") + theme(plot.title = element_text(hjust = 0.5, size = rel(0.9), face = "bold"))) +
     (study2_prim_aim1_eff_curve_plot$p_flanker.unp_comp + right_hand_themes) + 
     (study2_prim_aim1_eff_curve_plot$interference_flanker.vio_comp + right_hand_themes) + 
     (study2_prim_aim1_eff_curve_plot$interference_flanker.unp_comp + right_hand_themes) + plot_layout(ncol = 4)) +
  # Row 3: Explained variances
  ((study2_prim_aim1_variance_plot$p_flanker.vio_comp + ylim(0,100) + ggtitle("Explained variance (%)") + theme(plot.title = element_text(hjust = 0.5, size = rel(0.9), face = "bold"))) +
     (study2_prim_aim1_variance_plot$p_flanker.unp_comp + ylim(0,100) + right_hand_themes) + 
     (study2_prim_aim1_variance_plot$interference_flanker.vio_comp + ylim(0,100) + right_hand_themes) + 
     (study2_prim_aim1_variance_plot$interference_flanker.unp_comp + ylim(0,100) + right_hand_themes) + plot_layout(ncol = 4))
...
study2_aim3_plot <- 
  (study2_prim_aim3_effect_plot$value.vio_comp + ylim(-1,1) + ggtitle("Violence Exposure") + theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold")) +
  ...
  # Row 2: Effect curves
  ((study2_prim_aim3_eff_curve_plot$value.vio_comp + ggtitle("Effect curves") + theme(plot.title = element_text(hjust = 1, size = 14, face = "bold"))) +
     (study2_prim_aim3_eff_curve_plot$value.unp_comp + right_hand_themes) + plot_layout(ncol = 2)) +
  # Row 3: Explained variances
  ((study2_prim_aim3_variance_plot$value.vio_comp + ylim(0,100) + ggtitle("Explained variance (%)") + theme(plot.title = element_text(hjust = 1, size = 14, face = "bold"))) +
     (study2_prim_aim3_variance_plot$value.unp_comp + ylim(0,100) + right_hand_themes) + plot_layout(ncol = 2))

ggsave(plot = study2_aim3_plot, filename = "manuscript/figures/fig2.png", height=10, width = 9)
...
study2_aim1_results_study2_list <- study2_prim_aim1_effects_sum_study2 |> 
  ...
  select(decision, dv, iv, dv_iv, term, Std_Coefficient, p.value, Std_CI_low, Std_CI_high) |> 
  group_by(dv_iv) |> 
  summarise(
...
study2_aim1_results_list <- study2_prim_aim1_effects_sum |> 
  filter(!term %in% c("(Intercept)", "study.study1-intercept")) |> 
  select(decision, dv, iv, dv_iv, term, Std_Coefficient, p.value, Std_CI_low, Std_CI_high) |> 
  group_by(dv_iv) |> 
  summarise(
    median_effect_Main = round(median(Std_Coefficient),2),
...
study2_aim2_results_df <- study2_prim_aim2_effects_sum |> 
  ...
  left_join(
    bind_rows(
      study2_aim2_vio_boot_main1$result_sum |> mutate(term = "vio_comp"),
      study2_aim2_vio_boot_main2$result_sum |> mutate(term = "condition"),
      study2_aim2_vio_boot_int$result_sum |> mutate(term = "vio_comp:condition"),
      study2_aim2_unp_boot_main1$result_sum |> mutate(term = "unp_comp"),
      study2_aim2_unp_boot_int$result_sum |> mutate(term = "unp_comp:condition")
   ) |> unite(col = "dv_iv", c(dv,iv), sep = ".")
  ) |> 
  mutate(
    dv_iv = str_replace_all(dv_iv, "v_globloc.", ""),
    term = case_when(
      str_detect(term, ":") & dv_iv == "unp_comp" ~ "Interaction_unp",
      term == "condition" & dv_iv == "unp_comp" ~ "Condition_unp",
      str_detect(term, "comp") & dv_iv == "unp_comp"~ "Main_unp",
      
      str_detect(term, ":") & dv_iv == "vio_comp" ~ "Interaction_vio",
      term == "condition" & dv_iv == "vio_comp" ~ "Condition_vio",
      str_detect(term, "comp") & dv_iv == "vio_comp"~ "Main_vio"
    )) |> 
  mutate(across(
    -c(term, dv_iv, boot_p),
    ~formatC(.,  digits = 2, width = 3, flag = "0", format = 'f')
  ...
  mutate(boot_p = formatC(boot_p, digits = 3, width = 3, flag = "0", format = "f") %>% str_remove("^0") %>% str_replace_all("^.000$", "< .001")) |> 
  mutate(CI = paste0("[", median_CI_low, ", ", median_CI_high, "]")) |> 
  select(term, matches("median_effect"), ends_with("CI"), matches("p_sum"), boot_p) |>
...
study2_aim2_results_table <- study2_aim2_results_df |> 
  add_row(.before = 1, term = "Unpredictability") |> 
  add_row(.after = 4, term = "Violence Exposure") |> 
  mutate(across(everything(), ~ifelse(is.na(.), "", .))) |> 
  mutate( 
    term = str_replace_all(term, "_unp|_vio", " effect")) |> 
  flextable() |> 
  width(j = 1, width = .75) %>% 
  set_header_labels(
    term = ""
...
study2_aim2_simpslopes1_df <- 
  ...
  select(dv_iv, estimate, p.value, modx.value) |> 
  group_by(dv_iv, modx.value) |> 
  summarise(
  ...
  mutate(
    dv_iv = str_replace_all(dv_iv, "v_globloc.", ""),
    modx.value = ifelse(modx.value == -1, "Local", "Global")
  ) |> 
  unite("term", c(dv_iv, modx.value), sep = ".") |> 
  mutate(
    across(
      -c(term),
      ~formatC(.,  digits = 2, width = 3, flag = "0", format = 'f')
...
study2_aim2_simpslopes2_df <- 
  ...
  mutate(
    dv_iv = str_replace_all(dv_iv, "v_globloc.", ""),
    modx.value = ifelse(modx.value == -1, "Low", "High")
  ) |> 
  unite("term", c(dv_iv, modx.value), sep = ".") |> 
  mutate(
    across(
...
study2_aim3_results_df <- study2_prim_aim3_effects_sum |> 
  ...
  left_join(
    bind_rows(
      study2_aim3_vio_boot_main1$result_sum |> mutate(term = "vio_comp"),
      study2_aim3_vio_boot_main2$result_sum |> mutate(term = "task"),
      study2_aim3_vio_boot_int$result_sum |> mutate(term = "vio_comp:task"),
      study2_aim3_unp_boot_main1$result_sum |> mutate(term = "unp_comp"),
      study2_aim3_unp_boot_int$result_sum |> mutate(term = "unp_comp:task")
    ) |> unite(col = "dv_iv", c(dv,iv), sep = ".")
  ) |> 
  mutate(
    ...
    term = case_when(
      str_detect(term, ":") & dv_iv == "unp_comp" ~ "Interaction_unp",
      term == "task" & dv_iv == "unp_comp" ~ "Task_unp",
      str_detect(term, "comp") & dv_iv == "unp_comp"~ "Main_unp",
      
      str_detect(term, ":") & dv_iv == "vio_comp" ~ "Interaction_vio",
      term == "task" & dv_iv == "vio_comp" ~ "Task_vio",
      str_detect(term, "comp") & dv_iv == "vio_comp"~ "Main_vio"
    )) |> 
  mutate(across(
    -c(term, dv_iv, boot_p),
...
study2_aim3_simpslopes1_df <- 
  ...
  mutate(
    dv_iv = str_replace_all(dv_iv, "value.", ""),
    modx.value = ifelse(modx.value == -1, "Flanker", "GlobalLocal")
  ) |> 
  unite("term", c(dv_iv, modx.value), sep = ".") |> 
  mutate(
    across(
...
study2_aim3_simpslopes2_df <- 
  study2_prim_aim3_simslopes2 |> 
  select(dv_iv, estimate, p.value, modx.value) |> 
  group_by(dv_iv, modx.value) |> 
  summarise(
    median_ss = median(estimate),
...
study2_aim3_simslopes2_keys <- study2_aim3_simpslopes2_df  |> 
  group_keys(term)|> 
  pull()

study2_aim3_simslopes2_list <- study2_aim3_simpslopes2_df |> 
  group_split(term) |> 
  setNames(study2_aim3_simslopes2_keys)
