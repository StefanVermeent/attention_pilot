...
pilot_main_eff_ddm_table <- 
  ...
  compose(
    i = 1, j = 1,
    as_paragraph(as_b("Table 3. "), "Main and interaction effects of the effect of violence exposure on task performance."),
    part = "header"
  ) |>
...
pilot_expl_lm_eff_curve_plot <- unique(pilot_expl_lm_effects_sum$dv) |> 
  map(function(x) {
    ...
    pilot_expl_lm_effects_sum |> 
      ...
      geom_point(size = 3, shape = 19, show.legend = F) +
      geom_point(
        data = pilot_expl_lm_medians_sum[[x]],
        aes(y = med_effect_std, x = 32),
        shape = 1,
        size  = 3,
        fill  = "white",
      ...
      geom_text(
        aes(x = 32, y = 0.28, label = paste0(sum(p < .05)/n*100, " % of p-values < .05")),
        size = 5,
        show.legend = F,
        inherit.aes = F
      ) +
...
# Plot lm p-value curve
pilot_expl_lm_pvalues_plot <- unique(pilot_expl_lm_effects_sum$dv) |>
  map(function(x) {
    
    pvalues <- pilot_expl_lm_effects_sum |>
      filter(dv == x, parameter != "(Intercept)") |> 
      ungroup() |> 
      select(decision, p)  
...
pilot_expl_lm_variance_plot <- names(pilot_expl_lm_variance_sum) |>
  map(function(x) {
    
    pilot_expl_lm_variance_sum[[x]] |> 
      ggplot(aes(grp, percent, fill = grp)) +
      geom_bar(stat = "identity") +
      scale_fill_uchicago() +
      ylim(0, 100) +
  ...
  setNames(names(pilot_expl_lm_variance_sum))



pilot_expl_lmer_effect_plot <- names(pilot_expl_lmer_points_sum) |> 
  map(function(x) {
    
    pilot_expl_lmer_points_sum[[x]] |> 
      select(-iv) |> 
      rename(
        iv = level,
        condition = group
  ...
  setNames(names(pilot_expl_lmer_points_sum))

pilot_expl_lmer_eff_curve_plot <- unique(pilot_expl_lmer_effects_sum$dv) |> 
  map(function(x) {
    ...
    pilot_expl_lmer_effects_sum |> 
      ...
      geom_point(size = 1, shape = 19, show.legend = F) +
      geom_point(
        data = pilot_expl_lmer_medians_sum[[x]],
        aes(y = med_effect_std, x = 64),
        shape = 21,
        size  = 2.5,
        fill  = "white",
      ...
      geom_text(
        aes(x = 64, y = 0.18, label = paste0(sum(p < .05)/n*100, " % of p-values < .05")),
        size = 3,
        show.legend = F,
        inherit.aes = F
      ) +
...
pilot_expl_lmer_pvalues_plot <- names(pilot_expl_lmer_points_sum) |>
  map(function(x) {
    
    pvalues <- pilot_expl_lmer_points_sum[[x]] |> 
      select(decision, p) |> 
      distinct()
...
pilot_expl_lmer_variance_plot <- names(pilot_expl_lmer_variance_sum) |>
  map(function(x) {
    
    pilot_expl_lmer_variance_sum[[x]] |> 
      ggplot(aes(grp, percent, fill = grp)) +
      geom_bar(stat = "identity") +
      scale_fill_uchicago() +
      ylim(0, 100) +
...
save(txt_ivs_dist_pilot, txt_ivs_alpha_pilot, sample_pilot, pilot_prim_lm_effect_plot, pilot_prim_lm_eff_curve_plot, pilot_prim_lm_pvalues_plot, 
     pilot_prim_lm_variance_plot, pilot_prim_lmer_effect_plot, pilot_prim_lmer_eff_curve_plot, pilot_prim_lmer_pvalues_plot, pilot_prim_lmer_variance_plot, 
     pilot_expl_lm_eff_curve_plot, pilot_expl_lm_pvalues_plot, 
