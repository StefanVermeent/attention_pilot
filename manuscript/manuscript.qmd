...
Previous research has shown that people who grew up in adverse conditions tend to be slower on the Flanker Task, suggesting difficulty inhibiting distractions.
However, this pattern might result from slower general cognitive processes (e.g., reduced general processing speed), rather than the specific ability of inhibition.
We used Drift Diffusion Modeling in three studies---one pilot and two confirmatory studies---to better understand associations of adversity exposures with Flanker performance. 
Neither exposure to violence nor unpredictability were associated with inhibition.
In contrast, although mixed, exposure to violence and unpredictability were associated with lower strength of perceptual input; that is, how well someone can process the arrows in general.
Finally, lower strength of perceptual input was linked to a tendency to process information more holistically, focusing less on individual details.
Thus, researchers should be cautious when drawing inferences about inhibition abilities in people exposed to adversity based on response times alone.
...
We used an incremental preregistration approach across studies (for all preregistrations, code and materials, see [ANONYMIZED FOR REVIEW].
...
We conducted a power simulation using the *faux* package in R [@debruine_2021] to determine the minimally required number of participants for standardized regression coefficients of 0.10 and 0.15 (for details and simulation code, see [ANONYMIZED FOR REVIEW]).
...
However, there was one deviation from the preregistration: the multiverse analysis contained the same arbitrary decisions as the Pilot and Study 1, instead of a subset, as we preregistered (for details, [ANONYMIZED FOR REVIEW]).
See the supplemental materials (section 5) for figures summarizing *p*-distributions and the explained variance in the regression coefficients of each data cleaning decision. 
...
**Boundary separation:** Parameter of the Drift Diffusion Model and Shrinking Spotlight Model. Measures the amount of information a person requires before making a decision, determining the speed-accuracy trade-off (e.g., sacrificing speed in favor of higher accuracy).
