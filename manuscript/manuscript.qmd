...
All scripts, materials, and data supporting the results presented here are available on the article's GitHub repository [(https://stefanvermeent.github.io/attention_project/)](https://stefanvermeent.github.io/attention_project/).
...
Previous research has shown that people who grew up in adverse conditions tend to perform worse on cognitive tasks like the Flanker, which suggests that they have more difficulty inhibiting distractions.
Across an online Pilot and two online follow-up studies, we used Drift Diffusion Modeling to better understand performance differences on the Flanker in association with childhood adversity. 
Neither exposure to violence nor unpredictability were associated with inhibition.
In contrast, although mixed, results suggested that exposure to violence and unpredictability were associated with lower strength of perceptual input; that is, how well someone can process the arrows in general.
Finally, people exposed to adversity had a more holistic processing style, and this style was associated with lower strength of perceptual input.
Taken together, our findings underscore that researchers should be cautious when drawing inferences about inhibition abilities in people exposed to adversity based on response times alone.
...
We used an incremental preregistration approach across studies (for all preregistrations, code and materials, see <https://github.com/stefanvermeent/attention_project/preregistrations>).
...
We conducted a power simulation using the *faux* package in R [@debruine_2021] to determine the minimally required number of participants for standardized regression coefficients of 0.10 and 0.15 (for details and simulation code, see ([https://github.com/stefanvermeent/attention_project/preregistrations](https://github.com/stefanvermeent/attention_project/preregistrations)).
...
**Boundary separation:** Parameter from the Drift Diffusion Model and Shrinking Spotlight Model. Provides a measure of a person's level of response caution on binary decision-making tasks.
...
**Drift rate:** Parameter from the Drift Diffusion Model. Provides a measure of a person's speed of information accumulation on binary
...
**Multiverse analysis:** An analysis technique that systematically and transparently assesses the influence of arbitrary data cleaning decisions on analysis results.

<br>
<br>

**Non-decision time:** Parameter from the Drift Diffusion Model and Shrinking Spotlight Model. Provides a measure of the time it takes a person to prepare for the task at the start of the trial (before information processing) and the time it takes to execute a response (after a response boundary has been reached) on binary decision-making tasks.

<br>
<br>

**Perceptual input:** Parameter from the Shrinking Spotlight Model of Flanker performance. Provides a measure of how well a person can process the Flanker stimuli in general.
