...
spec_curves_unp_deg <- 
  map(c("DDM"), function(x){
    ...
    int_plots <- 
      ...
      scale_x_continuous(" ", breaks = c(1,2), labels = c("Standard","Degraded"), expand = c(.1,.1)) +
      scale_color_manual(values = pval_colors) +
...
# Put them together -------------------------------------------------------
secondary_unp_deg_fig <- 
  ggdraw() +
  draw_plot(
    plot_grid(
      spec_curves_unp_deg[[1]]$int,
      spec_curves_unp_deg[[1]]$eff_curve,
      spec_curves_unp_deg[[1]]$p_curve,
    #  spec_curves_unp_deg[[1]]$sample_sizes,
    #  spec_curves_unp_deg[[1]]$spec_grid,
      nrow  = 3,
      ncol  = 1, 
      align = "v", 
      axis  = "lr",
...
save(secondary_unp_deg_fig, file = here("data", "2_study1", "secondary_unp_deg_fig.RData"))
