...
#TODO: as soon as the standard version is done running, swap enhanced/degraded for enhanced
...
hddm_enh_data <- cleaned_data %>%
  select(id, vio_comp, unp_comp, a_hddm_std, matches("^(v|t)_hddm_diff_(std|enh)"),
...
hddm_deg_data <- cleaned_data %>%
  select(id, vio_comp, unp_comp, a_hddm_std, matches("^(v|t)_hddm_diff_(std|deg)"),
...
# Multiverse analysis - Standard vs. Enhanced -----------------------------

mult_hddm_grid_enh <- hddm_enh_data |>
...
mult_hddm_enh <- run_multiverse(mult_hddm_grid_enh)


mult_hddm_eff_enh <- mult_hddm_enh %>%
...
mult_hddm_simsl_enh <- mult_hddm_enh %>%
...
# Multiverse analysis - Standard vs. Degraded -----------------------------

mult_hddm_grid_deg <- hddm_deg_data |>
  multitool::add_variables("iv", vio_comp, unp_comp) |>
  multitool::add_variables("dv", a_hddm, v_hddm_diff, t_hddm_diff) |>
  multitool::add_filters(
    round(scale_factor,4) != 0.9007,
...
mult_hddm_deg <- run_multiverse(mult_hddm_grid_deg) 


mult_hddm_eff_deg <- mult_hddm_deg %>%
  unnest(c(lmer_fitted, specifications)) %>%
  select(decision, variables, filters, lmer_tidy) %>%
  unnest(c(filters, variables, lmer_tidy)) %>%
  select(-c(effect, group, statistic)) %>%
...
mult_hddm_eff_deg %>%
  group_by(iv, dv, term) %>%
  summarise(p = (sum(p.value<.05)/n())*100)

mult_hddm_simsl_deg <- mult_hddm_deg %>%
  unnest(sim_slopes_fitted) %>%
  select(decision, sim_slopes_tidy) %>%
  unnest(sim_slopes_tidy) %>%
  select(-c(statistic, mod2, mod2.value, term)) 
