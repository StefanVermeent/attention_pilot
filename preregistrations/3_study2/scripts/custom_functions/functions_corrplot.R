corr_table <- function (data, sample_size = T, use = "pairwise", method = "pearson", 
  ...
  if (flagged) {
    ps <- 
      corr_data$p %>% 
      as.data.frame() %>% 
      mutate(across(.cols = everything(), .fns = ~ my_ifelse(as.numeric(.x) <  .001 & !is.na(as.numeric(.x)), "***", .x))) %>% 
      mutate(across(.cols = everything(), .fns = ~ my_ifelse(as.numeric(.x) <  .01  & !is.na(as.numeric(.x)), "**", .x))) %>% 
