...
multiverse_data <- cleaned_data %>%
  ...
  mutate(rt_diff = flanker_congruent_rt_raw - flanker_incongruent_rt_raw) %>%
...
spec_grid <- 
  ...
  left_join(
    tribble(
      ...
      "dv_type",         2,           "rt_diff",                                                       "*M*~Congr~ - *M*~Incongr~",
      "dv_type",         3,           "a_flanker",                                                     "Boundary separation (a)",
      "dv_type",         4,           "t0_flanker",                                                    "Non-decision time (t0)",
      "dv_type",         5,           "p_flanker",                                                     "Perceptual input (p)",
      "dv_type",         6,           "sda_flanker",                                                   "Initial width (sda)",
      "dv_type",         7,           "rd_flanker",                                                    "Shrinking rate (rd)",
      "dv_type",         8,           "interference",                                                  "sda / rd (interference)",
...
# Save environment --------------------------------------------------------
save(multi_data_list, file = here("data", "1_pilot", "1_primary_analyses", "flanker", "1_multiverse_objects.Rdata"))
