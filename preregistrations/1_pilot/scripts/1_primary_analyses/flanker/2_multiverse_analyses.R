...
# Data
load(here("data", "1_pilot", "1_primary_analyses", "flanker", "1_multiverse_objects.Rdata"))
...
primary_vio_flanker <- 
  ...
  future_map(function(x) {
    ...
    if(dv %in% c("rt_raw", "rt_log")) {
      ...
      # Model object
      mod      <- lmer(str_c(dv, " ~ violence_composite_c * condition_sum + (1|id)"), data=data_raw)
      # Tidy Model
    ...
    if(dv %in% c("rt_diff", "a_flanker", "t0_flanker", "p_flanker", "rd_flanker", "sda_flanker", "interference")) {
      
      data_ssp <- x$data_analysis$data_ssp
      
     
      # Model object
      mod      <- lm(str_c(dv, " ~ violence_composite_c"), data=data_ssp)
      # Tidy Model
      mod_tidy <- broom.mixed::tidy(mod) %>% rename_all(~paste0("mod_",.))
      #Standardized coefficients
      mod_std <- standardize_parameters(mod)
...
save(primary_vio_flanker, file=here("data", "1_pilot", "1_primary_analyses", "flanker", "2_multiverse_results.Rdata"))
