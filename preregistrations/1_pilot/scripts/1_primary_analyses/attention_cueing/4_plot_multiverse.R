...
spec_curves_interaction <- 
  map(c("Raw scores", "DDM"), function(x){
    ...
    eff_curve <- 
      ...
      geom_label(
        data = medians,
        aes(y = median_dbl, label = median_chr, x = 64),
        nudge_y = .055,
        size = 2.5,
        show.legend = F,
        inherit.aes = F
...
primary_cueing_fig1 <- 
  ...
  draw_plot(
    plot_grid(
      spec_curves_interaction[[1]]$int_plots,
      spec_curves_interaction[[1]]$eff_curve,
      spec_curves_interaction[[1]]$p_curve,
      spec_curves_interaction[[1]]$sample_sizes,
 ...
 # draw_label("Attention Cueing: Violence exposure X condition interaction on raw scores", x = 0.6, y = .975, hjust = .5, vjust = 0, fontface = "bold")

primary_cueing_fig2 <- 
  ggdraw() +
  draw_plot(
    plot_grid(
      spec_curves_interaction[[2]]$int_plots,
      spec_curves_interaction[[2]]$eff_curve,
      spec_curves_interaction[[2]]$p_curve,
      spec_curves_interaction[[2]]$sample_sizes,
   ...
   # rel_heights = c(.25, .15, .2, .4)
     ...
     draw_plot_label(c("a","b","c", "d"), x = 1, y = c(.95, .65, .35, .15), size = 10, vjust = 1, hjust = 1), 
   x = 0, y = 0, width = 1, height = .95
  )# + 
 # draw_label("Attention Cueing: Violence exposure X condition interaction on DDM parameters", x = 0.6, y = .975, hjust = .5, vjust = 0, fontface = "bold")
...
primary_cueing_fig3 <- 
  ...
  draw_plot(
    plot_grid(
     # spec_curves[[1]]$int_plots,
      spec_curves_maineffect[[1]]$eff_curve,
      spec_curves_maineffect[[1]]$p_curve,
      spec_curves_maineffect[[1]]$sample_sizes,
      #  spec_curves[[1]]$spec_grid,
      nrow  = 4,
      ncol  = 1, 
      align = "v", 
  ...
  draw_label("Attention Cueing: Violence exposure main effect on raw scores", x = 0.6, y = .975, hjust = .5, vjust = 0, fontface = "bold")
  ...
  draw_plot(
    plot_grid(
     # spec_curves[[2]]$int_plots,
      spec_curves_maineffect[[2]]$eff_curve,
      spec_curves_maineffect[[2]]$p_curve,
      spec_curves_maineffect[[2]]$sample_sizes,
      #   spec_curves[[2]]$spec_grid,
      nrow  =3,
      ncol  = 1, 
      align = "v", 
      ...
      draw_plot_label(c("a","b","c"), x = 1, y = c(.95, .7, .5), size = 10, vjust = 1, hjust = 1), 
    x = 0, y = 0, width = 1, height = .95
  ) + 
  draw_label("Attention Cueing: Violence exposure X condition interaction on DDM parameters", x = 0.6, y = .975, hjust = .5, vjust = 0, fontface = "bold")


cueing_simple_slopes_table <- primary_simple_slopes %>%
  group_by(dv,level) %>%
  summarise(
  ...
  map(function(x) {
    x %>%
      ...
      select(dv, median_b_neutral, percent_p_neutral, median_b_cued, percent_p_cued, median_b_low, percent_p_low, median_b_high, percent_p_high)
  }) %>%
  bind_rows %>%
  mutate(across(!dv, ~round(., 2))) %>%
  arrange(factor(dv, levels = c("RT", "RT (log)", "Drift rate (v)", "Boundary separation (a)", "Non-decision time (t0)"))) %>%
  flextable() %>%
  set_header_labels(
    values = list(
      dv = "",
      median_b_neutral = "b (neutral)",
      median_b_cued = "b (cued)",
      median_b_low = "b (low)",
...
save(primary_cueing_fig1, primary_cueing_fig2, cueing_simple_slopes_table, file = here("preregistrations", "1_pilot", "presentations", "primary_figures_cueing.Rdata"))
