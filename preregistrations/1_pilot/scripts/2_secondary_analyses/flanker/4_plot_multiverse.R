...
load(here("data", "1_pilot", "2_secondary_analyses", "flanker", "3_multiverse_extracted_effects.Rdata"))
...
secondary_effects_flanker %<>%
  filter(dv != "Shrinking rate (rd)") %>%
  mutate(dv = factor(dv, levels = c("RT", "RT (log)", "Perceptual input (p)", "Boundary separation (a)", 
                                    "Non-decision time (t0)", "Initial width (sda)", "sda / rd (interference)")))
...
spec_curves_raw <- 
  map(c("Raw scores"), function(x){
    ...
    int_points <- 
      ...
      mutate(dv = factor(dv, levels = c("RT", "RT (log)", "Perceptual input (p)", "Boundary separation (a)", 
                                        "Non-decision time (t0)", "Initial attentional width (sda)", "Rate of spotlight shrinking (rd)"))) %>%
      mutate(
        mod_sig   = ifelse(mod_sig != "non", "pos-sig", mod_sig),
    ...
    int_plots <- 
      ...
      geom_line(alpha = 0.7, size = .5, show.legend = F) +
...
# Put them together -------------------------------------------------------
secondary_flanker_fig1 <- 
  ggdraw() +
  draw_plot(
    plot_grid(
      spec_curves_raw[[1]]$int_plots,
      spec_curves_raw[[1]]$eff_curve,
      spec_curves_raw[[1]]$p_curve,
      spec_curves_raw[[1]]$sample_sizes,
  ...
  draw_label("Flanker: Unpredictability X condition interaction on raw scores", x = 0.6, y = .975, hjust = .5, vjust = 0, fontface = "bold")

secondary_flanker_fig2 <- 
  ggdraw() +
  draw_plot(
    plot_grid(
      spec_curves_ssp[[1]]$eff_curve,
      spec_curves_ssp[[1]]$p_curve,
      spec_curves_ssp[[1]]$sample_sizes,
  ...
  draw_label("Flanker: Unpredictability main effect on DDM parameters", x = 0.6, y = .975, hjust = .5, vjust = 0, fontface = "bold")


save(secondary_flanker_fig1, secondary_flanker_fig2, file = here("preregistrations", "1_pilot", "presentations", "secondary_figures_flanker.Rdata"))
