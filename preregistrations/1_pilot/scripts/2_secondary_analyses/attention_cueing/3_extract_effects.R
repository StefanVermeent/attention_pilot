...
# Load results
load(here("data", "1_pilot", "secondary_analyses", "attention_cueing", "2_multiverse_results.Rdata"))
...
secondary_effects_cueing <- 
  ...
  mutate(
    ...
    pval_prop  = (sum(mod_p.value < .05)/(640/5)) * 100
  ) %>% 
  ungroup() %>% 
  mutate(
    ...
    mod_term_label = case_when(str_detect(mod_term, "Intercept")  ~ "Intercept",
                               mod_term == "unpredictability_composite_c" ~ "unpredictability exposure",
...
save(
  ...
  secondary_effects_points,
  secondary_simple_slopes,
  file = here("data", "1_pilot", "secondary_analyses", "attention_cueing", "3_multiverse_extracted_effects.Rdata")
)
