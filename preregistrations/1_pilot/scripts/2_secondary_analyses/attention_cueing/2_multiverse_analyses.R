...
# Data
load(here("data", "1_pilot", "secondary_analyses", "attention_cueing", "1_multiverse_objects.Rdata"))

cores = parallel::detectCores()
plan("multisession", workers = cores-4)

tic()
secondary_unp_cueing <- 
  multi_data_list %>%
  future_map(function(x) {
   ...
   #  formula <- as.formula(str_glue("{dv} ~ unpredictability_composite_c * condition_sum + (1|id)"))
    
     # Model object
     mod      <- lmer(str_c(dv, " ~ unpredictability_composite_c * condition_sum + (1|id)"), data=data)
...
save(secondary_unp_cueing, file=here("data", "1_pilot", "secondary_analyses", "attention_cueing", "2_multiverse_results.Rdata"))
