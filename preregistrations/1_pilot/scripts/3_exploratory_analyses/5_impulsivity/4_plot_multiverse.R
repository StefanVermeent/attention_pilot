...
load(here("data", "1_pilot", "3_exploratory_analyses", "4_impulsivity", "3_multiverse_extracted_effects.Rdata"))
 ...
 # draw_label("Flanker: Unpredictability X condition interaction on raw scores", x = 0.6, y = .975, hjust = .5, vjust = 0, fontface = "bold")
  ...
  draw_plot(
    plot_grid(
      ...
      align = "v", 
      axis  = "lr",
      rel_heights = c(.4, .2, .2, .2)
    ) +
      draw_plot_label(c("a","b","c", "d"), x = 1, y = c(.95, .6, .3), size = 10, vjust = 1, hjust = 1), 
    x = 0, y = 0, width = 1, height = .95
  )

save(exploratory_flanker_fig1, exploratory_flanker_fig2, file = here("preregistrations", "1_pilot", "presentations", "exploratory_figures_flanker.Rdata"))
