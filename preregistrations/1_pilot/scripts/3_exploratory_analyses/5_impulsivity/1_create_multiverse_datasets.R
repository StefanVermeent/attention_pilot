...
multiverse_data <- cleaned_data %>%
  select(id, scale_factor, fullscreenenter, fullscreenexit, meta_captcha, attention_interrupt_sum, att_noise, impuls_mean,
...
multi_data_list <-
  ...
  map(function(x){
    ...
    data_raw <-
      ...
      mutate(
        impuls_mean_c = scale(impuls_mean, scale = FALSE) %>% as.numeric,
...
# Save environment --------------------------------------------------------
save(multi_data_list, file = here("data", "1_pilot", "3_exploratory_analyses", "4_impulsivity", "1_multiverse_objects.Rdata"))
