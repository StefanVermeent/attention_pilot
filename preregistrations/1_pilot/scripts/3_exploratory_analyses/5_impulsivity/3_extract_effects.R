...
exploratory_effects_flanker <- 
  ...
  mutate(
    ...
    mod_term_unique = case_when(mod_term == "impuls_mean_c" ~ "impuls",
                                mod_term == "impuls_mean_c:condition_sum" ~ "impuls~symbol('\\264')~Task-Condition",
                                T ~ mod_term_label),
...
save(
  ...
  exploratory_effects_points,
  exploratory_simple_slopes,
  file = here("data", "1_pilot", "3_exploratory_analyses", "4_impulsivity", "3_multiverse_extracted_effects.Rdata")
)
