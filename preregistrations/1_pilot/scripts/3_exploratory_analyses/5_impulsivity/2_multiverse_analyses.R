...
# Data
load(here("data", "1_pilot", "3_exploratory_analyses", "4_impulsivity", "1_multiverse_objects.Rdata"))
...
exploratory_impuls_flanker <- 
  ...
  future_map(function(x) {
    ...
    if(dv %in% c("rt_raw", "rt_log")) {
      ...
      # Model object
      mod      <- lmer(str_c(dv, " ~ impuls_mean_c * condition_sum + (1|id)"), data=data_raw)
      # Tidy Model
      ...
      mod_effects <- ggpredict(mod, terms = c("impuls_mean_c [-1,1]", "condition_sum [-1,1]"))
      ss_task <- sim_slopes(mod, pred = "impuls_mean_c", modx = "condition_sum", modx.values = c(-1,1))
      ss_unp  <- sim_slopes(mod, pred = "condition_sum", modx = "impuls_mean_c", modx.values = c(-1,1))
    ...
    if(dv %in% c("a_flanker", "t0_flanker", "p_flanker", "rd_flanker", "sda_flanker", "interference")) {
      ...
      # Model object
      mod      <- lm(str_c(dv, " ~ impuls_mean_c"), data=data_ssp)
      # Tidy Model
      mod_tidy <- broom.mixed::tidy(mod) %>% rename_all(~paste0("mod_",.))
      #Standardized coefficients
      mod_std <- standardize_parameters(mod)
...
save(exploratory_impuls_flanker, file=here("data", "1_pilot", "3_exploratory_analyses", "4_impulsivity", "2_multiverse_results.Rdata"))
