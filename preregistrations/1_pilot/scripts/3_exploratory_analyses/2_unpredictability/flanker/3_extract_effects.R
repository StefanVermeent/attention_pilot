...
# Load results
load(here("data", "1_pilot", "3_exploratory_analyses", "2_unpredictability", "flanker", "2_multiverse_results.Rdata"))
...
exploratory_effects_unp_flanker <- 
 exploratory_flanker %>% 
  ...
  mutate(
    ...
    mod_term_unique = case_when(mod_term == "iv" ~ "unp",
                                mod_term == "iv" ~ "unp~symbol('\\264')~Task-Condition",
                                T ~ mod_term_label),
