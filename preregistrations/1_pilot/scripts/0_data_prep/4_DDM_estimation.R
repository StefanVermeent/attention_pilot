...
## Model 1: Seven-parameter model: v, a, t0, z, and inter-trial variability parameters
...
## Model 2: Four-parameter model: v, a, t0 and d
...
## Model 3: Three-parameter model: v, a, and t0
...
change_sim_params <- simulate_DDM_parameters(data = change_DDM_results_mod3, nsim = 5000)
...
change_simulation_fit_vector <- read_DDM(task = "change", model_version = "_mod3") %>%
  select(fit) %>%
  pull() 


change_DDM_results_mod3 %<>%
...
## 4. 3-parameter model (z fixed to 0.5); a is constrained to be equal across conditions
