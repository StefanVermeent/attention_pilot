...
table_demographics <- bind_rows(sex_at_birth, gender, ethnicity, education, social_class) %>%
  ...
  add_row(.before = 1, "var" = "Mean age (SD)", percentage = paste0(round(mean(cleaned_data$dems_age, na.rm=T),2), " (", round(sd(cleaned_data$dems_age, na.rm=T),2), ")")) %>%
