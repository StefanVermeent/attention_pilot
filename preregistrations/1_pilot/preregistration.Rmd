...
load(here("preregistrations", "1_pilot", "staged_objects.Rmd"))
```
...
We computed several unweighted unpredictability variables to be able to assess how they would relate to the dependent variables as well as to each other: 1) overall perceived unpredictability variable containing all eight items of the perceived unpredictability scale (*M* = `r txt_ivs$unp_mean$mean`, *SD* = = `txt_ivs$unp_mean$sd`, $\alpha$ = `r #TBD `); 2) an overall QUIC variable by averaging all 37 items (*M* = `r txt_ivs$quic_total_mean$mean`, *SD* = = `r txt_ivs$quic_total_mean$sd`, $\alpha$ = `r #TBD `); 3) five separate QUIC subscales by averaging their sub-items (Parental monitoring and involvement: *M* = `r txt_ivs$quic_monitoring_mean$mean`, *SD* = `r txt_ivs$quic_monitoring_mean$sd`, $\alpha$ = `r #TBD `; Parental predictability: *M* = `r txt_ivs$quic_par_predict_mean$mean`, *SD* = `r txt_ivs$quic_par_predict_mean$sd`, $\alpha$ = `r #TBD `; Parental environment: *M* = `r txt_ivs$quic_par_env_mean$mean`, *SD* = `r txt_ivs$quic_par_env_mean$sd`, $\alpha$ = `r #TBD `; Physical environment: *M* = `r txt_ivs$quic_phys_env_mean$mean`, *SD* = `r txt_ivs$quic_phys_env_mean$sd`, $\alpha$ = `r #TBD `; Safety and security: *M* = `r txt_ivs$quic_safety_mean$mean`, *SD* = `r txt_ivs$quic_safety_mean$sd`, $\alpha$ = `r #TBD `). See also [below](#efa) for details on a factor analysis based on the individual items across scales.

One additional scale measured the stability of the family and social environment. On a scale of 1 (the same all the time) to 5 (constant and rapid changes), participants indicated how often the following aspects of their family and social environment changed before age 13: 1) economic status, 2) family environment, 3) childhood neighborhood environment, and 4) childhood school environment. 
...
The full experiment was expected to take approximately 35 minutes. Participants were paid £4.38 upon completing the full experiment.
...
### Change Detection
Violence exposure did not have an effect on the speed of change detection (see Figure 1). However, participants with high violence exposure were less accurate ($\beta$~median~  = `r #TBD`, `r #TBD`% of *p*s < .05).

we found a negative main effect of violence exposure on the drift rate ($\beta$~median~  = `r #TBD`, `r #TBD`% of *p*s < .05), indicating that participants with more violence exposure showed slower information processing.

### Attention Cueing Task
We did not find a significant interaction effect between violence exposure and condition on raw Attention Cueing performance (see Figure 2. There was a significant main effect of violence exposure (($\beta$~median~  = `r #TBD`) under `r #TBD`% and `r #TBD`% of analytic decisions on raw RTs and log-transformed RTs, respectively (not shown in this figure), indicating that participants with high violence exposure generally performed slower on the task.

The DDM analysis uncovered a significant interaction effect between violence exposure and task condition on the drift rate ($\beta$~median~  = `r #TBD`, `r #TBD`% of *p*s < .05). Simple slopes analysis suggested that on cued trials, high violence exposure participants tended to have a lower drift rate than low violence exposure participants (`r #TBD`% ps < .05). This difference disappeared on neutral trials, with both groups showing comparable drift rates (0% ps < .05) (See Table 2).

We did not find significant interaction effects for boundary separation and non-decision time, although we did find significant main effects of violence exposure on boundary separation ($\beta$~median~  = `r #TBD`, `r #TBD`% of *p*s < .05) and non-decision time ($\beta$~median~  = `r #TBD`, `r #TBD`% of *p*s < .05), suggesting that participants with more violence exposure had more conservative decision boundaries and were slower to start the decision-making process (possibly reflecting slower eye-movements towards the targets).


### Flanker Task
We found a significant interaction effect between violence exposure and task condition on raw Flanker performance in `r #TBD`% and `r #TBD`% of analytic decisions for raw RTs and log-transformed RTs, respectively (see Figure 3). Simple slopes analyses showed that under `r #TBD`% of analytic decisions, participants with more violence exposure were slightly slower on congruent trials. This difference disappeared across all analytic decisions on incongruent trials.

The DDM analyses showed a combination of parameter differences that suggest a compensatory mechanism in people with more violence exposure. First, people with more violence exposure extracted lower-quality perceptual information (p) from the flankers ($\beta$~median~  = `r #TBD`, `r #TBD`% of *p*s < .05), which is analogous to the lower drift rates on the other tasks. Thus, they needed more time to gather enough information to reach the decision boundary. Second, the initial width of the attention spotlight (sda) did not differ across levels of violence exposure, which is not in line with our expectations of a broader, more diffuse attention style in people from adversity.
Third, people with more violence exposure had a higher shrinking rate rd ($\beta$~median~  = `r #TBD`, `r #TBD`% of *p*s < .05), meaning that they were faster at narrowing down their attention towards the central arrow (thereby decreasing flanker interference quicker, as shown by the sda/rd ratio). Judging from the interaction effect on raw RTs, this higher shrinking rate seems to offset the negative effect of lower perceptual input on incongruent trials. Thus, contrary to expectations, participants with more violence exposure seem to experience less interference from the flanker arrows.

## Secondary analyses

### Factor structure of unpredictability items
